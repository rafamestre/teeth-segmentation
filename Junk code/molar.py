...
def sliding_window(image,imageEdges, stepSize, windowSize):
	# slide a window across the image
	for y in xrange(0, image.shape[0], stepSize):
		for x in xrange(0, image.shape[1], stepSize):
			# yield the current window GENERATOR (can only be iterated once)
			yield (x, y, image[y:y + windowSize[1], x:x + windowSize[0]], 
			             imageEdges[y:y + windowSize[1], x:x + windowSize[0]])

def sliding_windowSimple(image, stepSize, windowSize):
	# slide a window across the image
	for y in xrange(0, image.shape[0], stepSize):
		for x in xrange(0, image.shape[1], stepSize):
			# yield the current window GENERATOR (can only be iterated once)
...
if __name__ == '__main__':
    ...
    #database resizes the picture sof the molars with the same size nbPixels x nbPixels

    directoryPositive = '_Data/Radiographs/PositiveMolars/'
    directoryNegative = '_Data/Radiographs/NegativeMolars/'
    database(nbPixelsHeight, nbPixelsWide, directoryPositive,directoryNegative)
    
    directoryPositiveEdges = '_Data/Radiographs/PositiveMolarsEdges/'
    directoryNegativeEdges = '_Data/Radiographs/NegativeMolarsEdges/'
    database(nbPixelsHeight, nbPixelsWide, directoryPositiveEdges,directoryNegativeEdges)

    #X is the vector with the pixels of the images for PCA
        
    X = createX('_Data/Radiographs/PositiveMolars/Rescaled',nbPixelsHeight*nbPixelsWide)
    XNonMolar = createX('_Data/Radiographs/NegativeMolars/Rescaled',nbPixelsHeight*nbPixelsWide)
    
    XEdges = createX('_Data/Radiographs/PositiveMolarsEdges/Rescaled',nbPixelsHeight*nbPixelsWide)
    XNonMolarEdges = createX('_Data/Radiographs/NegativeMolarsEdges/Rescaled',nbPixelsHeight*nbPixelsWide)

        
    #pca(X) makes PCA of the X vector and returns eigenvalues, eigenvectors and mean
    
    [eigenvaluesMolar, eigenvectorsMolar, muMolar] = pca(X)
    [eigenvaluesNonMolar, eigenvectorsNonMolar, muNonMolar] = pca(XNonMolar)
    [eigenvaluesMolarEdges, eigenvectorsMolarEdges, muMolarEdges] = pca(XEdges)
    [eigenvaluesNonMolarEdges, eigenvectorsNonMolarEdges, muNonMolarEdges] = pca(XNonMolarEdges)
    ...
    for (x, y, window, windowEdges) in sliding_window(gray, dilation, stepSize=12, windowSize=(winW, winH)):
	...
	if window.shape[0] != winH or window.shape[1] != winW:
	   ...
	   time.sleep(0.025)  
        ...
        grayWindowEdges = cv2.equalizeHist(windowEdges)
        
        windowX = grayWindow.flatten()    
        windowXEdges = grayWindowEdges.flatten()
	
        #project image on the subspace of the molar teeth
        
        YMolarTest = project(eigenvectorsMolar, windowX, muMolar)
        XMolarTest = reconstruct(eigenvectorsMolar, YMolarTest, muMolar)
        
        YMolarEdgesTest = project(eigenvectorsMolarEdges, windowXEdges, muMolarEdges)
        XMolarEdgesTest = reconstruct(eigenvectorsMolarEdges, YMolarEdgesTest, muMolarEdges)

        
        YNonMolarTest = project(eigenvectorsNonMolar, windowX, muNonMolar)
        XNonMolarTest = reconstruct(eigenvectorsNonMolar, YNonMolarTest, muNonMolar)
        
        YNonMolarEdgesTest = project(eigenvectorsNonMolarEdges.real, windowXEdges, muNonMolarEdges)
        XNonMolarEdgesTest = reconstruct(eigenvectorsNonMolarEdges, YNonMolarEdgesTest, muNonMolarEdges)

        
        windowX.resize((windowX.shape[0],1))
        windowXEdges.resize((windowX.shape[0],1)) 	


        diffMolar = np.linalg.norm(windowX - XMolarTest) - np.linalg.norm(windowX - XNonMolarTest)
        diffEdges = np.linalg.norm(windowXEdges - XMolarEdgesTest) - np.linalg.norm(windowXEdges - XNonMolarEdgesTest)
        diff = diffEdges + diffMolar
        #diff = diffEdges
    ...
    for (x, y, window) in sliding_windowSimple(cropped, stepSize=5, windowSize=(winW, winH)):
