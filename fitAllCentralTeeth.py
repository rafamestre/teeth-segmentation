...
def activeShape(gray, dilation, scalingLevel, eigenvector, b, mu, reconstructedLandmarks, wait=True):
    ...
    #Depending on the scaling level, different edge paramteres are used
    if scalingLevel == 4:
    ...
    while convergence == False:
        ...
        #I put constrains in the window, so that the shape doesn't migrate 
        for even in range(0,nbLandmarks-1,2):
...
def fitTeeth(point,tooth,file_name,width,height,directoryIm='_Data/Radiographs/extra/', wait=True,lower=False):
    ...
    for even in range(0, nbLandmarks,2):
        meanPosition[even] = meanPosition[even] + gray.shape[1]/2
        meanPosition[even+1] = meanPosition[even+1] + gray.shape[0]/2
    ...
    center = getCentroid(reconstructedLandmarks)
    center1 = getCentroid(reconstructedLandmarks[0:nbLandmarks/4],nbLandmarks/4)
    center2 = getCentroid(reconstructedLandmarks[nbLandmarks/4:nbLandmarks/2],nbLandmarks/4)
    center3 = getCentroid(reconstructedLandmarks[nbLandmarks/2:3*nbLandmarks/4],nbLandmarks/4)
    center4 = getCentroid(reconstructedLandmarks[3*nbLandmarks/4:nbLandmarks],nbLandmarks/4)
    
    return center, center1,center2, center3, center4, reconstructedLandmarks
    
    
def getPCAcomponents(incisor):
    ...
    b = project(eigenvector,alignedLandmarks[2,:],mu)
    reconstruction = reconstruct(eigenvector,b,mu)
        
    return eigenvector,eigenvalue,mu
