...
def activeShape(gray, dilation, scalingLevel, eigenvector, b, mu, reconstructedLandmarks, wait=True):
    ...
    #Depending on the scaling level, I used different edge detection paramters
    if scalingLevel == 4:
    ...
    while convergence == False:
        ...
        if convergenceSum < scalingLevel/6.0:
            convergence = True          
        else:
...
def fitTeeth(point,center,teeth,file_name,width,height,directoryIm='_Data/Radiographs/extra/', wait=True,lower=False):
    ...
    return center, reconstructedLandmarks
